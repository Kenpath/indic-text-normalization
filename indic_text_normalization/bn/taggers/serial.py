...
from indic_text_normalization.bn.utils import get_abs_path, load_labels
...
class SerialFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst, deterministic: bool = True, lm: bool = False):
        ...
        values with delimiters are handled by default).
            ...
            c325b -> tokens { cardinal { integer: "c তিন দুই পাঁচ b" } }
        """
        if deterministic:
            # For Bengali, use final_graph for numbers
        ...
        serial_graph |= (
            ...
            + (pynini.cross("^2", " বর্গ") | pynini.cross("^3", " ঘন")).optimize()
        )
