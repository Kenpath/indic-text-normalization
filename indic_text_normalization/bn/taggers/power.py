...
class PowerFst(GraphFst):
    ...
    Finite state transducer for classifying powers/exponents with superscripts, e.g.
        "10⁻⁷" -> power { base: "দশ" sign: "মাইনাস" exponent: "সাত" }
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Base number (regular digits - Bengali or Arabic)
        bn_base_input = pynini.closure(NEMO_BN_DIGIT, 1)
        bn_base = pynini.compose(bn_base_input, cardinal_graph).optimize()
        
        arabic_base_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_to_bn = pynini.closure(
            pynini.string_map([
                ("0", "০"), ("1", "১"), ("2", "২"), ("3", "৩"), ("4", "৪"),
                ("5", "৫"), ("6", "৬"), ("7", "৭"), ("8", "৮"), ("9", "৯")
            ])
        ).optimize()
        arabic_base = pynini.compose(arabic_base_input, arabic_to_bn @ cardinal_graph).optimize()
        
        base_number = bn_base | arabic_base
        ...
        optional_sign = pynini.closure(
            pynutil.insert('sign: "')
            + (
                pynini.cross(NEMO_SUPERSCRIPT_MINUS, "মাইনাস")
                | pynini.cross(NEMO_SUPERSCRIPT_PLUS, "প্লাস")
            )
            + pynutil.insert('"')
            + insert_space,
            0,
        ...
        exponent_value = pynini.compose(
            superscript_number,
            pynini.closure(superscript_to_digit) @ arabic_to_bn @ cardinal_graph
        ).optimize()
