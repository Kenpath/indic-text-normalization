...
class PostProcessingFst:
    ...
    def __init__(self, cache_dir: str = None, overwrite_cache: bool = False):
        ...
        if cache_dir is not None and cache_dir != "None":
            ...
            far_file = os.path.join(cache_dir, "bn_tn_post_processing.far")
    ...
    def get_punct_postprocess_graph(self):
        ...
        Removes extra spaces before common punctuation marks like period, question mark, etc.
        """
        # Common punctuation marks that shouldn't have a space before them
        punct_marks = pynini.union(".", "?", "!", ";", ":", "।", ",")
        
        # Create a rule to delete space before punctuation
        delete_space_before_punct = pynini.cdrewrite(
            pynini.cross(" ", ""),  # Delete single space
            "",  # No left context required
            punct_marks,  # Before punctuation
            NEMO_SIGMA
        )
        
        graph = delete_space_before_punct.optimize()
        return graph
