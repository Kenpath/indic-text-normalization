...
from indic_text_normalization.bn.graph_utils import NEMO_NOT_QUOTE, GraphFst, delete_space, insert_space


class MathFst(GraphFst):
    """
    Finite state transducer for verbalizing math expressions, e.g.
        math { left: "এক" operator: "সমান" right: "দুই" } -> এক সমান দুই
        math { left: "এক" operator: "প্লাস" right: "দুই" } -> এক প্লাস দুই
    ...
    def __init__(self, deterministic: bool = True):
        ...
        # Operator with number (e.g., "+5" -> "প্লাস পাঁচ")
        operator_number_expression = operator + insert_space + right

        # Number with operator (e.g., "5*" -> "পাঁচ গুণ")
        number_operator_expression = left + insert_space + operator

        # Standalone operator (e.g., "+" -> "প্লাস", "?" -> "প্রশ্ন চিহ্ন")
