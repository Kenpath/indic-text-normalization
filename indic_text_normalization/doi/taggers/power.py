...
class PowerFst(GraphFst):
    ...
    Finite state transducer for classifying powers/exponents with superscripts, e.g.
        "10⁻⁷" -> power { base: "दह" sign: "ऋणात्मक" exponent: "सत्त" }
        "2³" -> power { base: "दो" exponent: "त्रै" }
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Base number (regular digits - Dogri or Arabic)
        dogri_base_input = pynini.closure(NEMO_HI_DIGIT, 1)
        dogri_base = pynini.compose(dogri_base_input, cardinal_graph).optimize()
        
        arabic_base_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_to_dogri = pynini.closure(
            pynini.string_map([
                ("0", "०"), ("1", "१"), ("2", "२"), ("3", "३"), ("4", "४"),
                ("5", "५"), ("6", "६"), ("7", "७"), ("8", "८"), ("9", "९")
            ])
        ).optimize()
        arabic_base = pynini.compose(arabic_base_input, arabic_to_dogri @ cardinal_graph).optimize()
        
        base_number = dogri_base | arabic_base
        ...
        exponent_value = pynini.compose(
            superscript_number,
            pynini.closure(superscript_to_digit) @ arabic_to_dogri @ cardinal_graph
        ).optimize()
