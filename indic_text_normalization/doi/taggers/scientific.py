...
from indic_text_normalization.doi.graph_utils import GraphFst, NEMO_DIGIT, NEMO_HI_DIGIT, insert_space
...
class ScientificFst(GraphFst):
    ...
    Supported examples (Dogri):
      - "10.1-e5" -> scientific { mantissa: "दह दशमलव इक्क" exponent: "पंज" }
      - "10.1e-5" -> scientific { mantissa: "दह दशमलव इक्क" sign: "ऋणात्मक" exponent: "पंज" }

    Verbalizer format (see `doi/verbalizers/scientific.py`):
      mantissa + " गुणा दह पावर " + [sign] + exponent
    """
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        dogri_int = pynini.compose(pynini.closure(NEMO_HI_DIGIT, 1), cardinal_graph).optimize()
        arabic_int = pynini.compose(pynini.closure(NEMO_DIGIT, 1), arabic_to_dogri_number @ cardinal_graph).optimize()
        integer_graph = (dogri_int | arabic_int).optimize()
        ...
        arabic_frac = pynini.compose(
            ...
            arabic_to_dogri_number @ (digit_word_graph + pynini.closure(insert_space + digit_word_graph)),
        ).optimize()
