...
MAG_POINT_FIVE = ".५"  # .5
MAG_ONE_POINT_FIVE = "१.५"  # 1.5
MAG_TWO_POINT_FIVE = "२.५"  # 2.5
MAG_DECIMAL_25 = ".२५"  # .25
MAG_DECIMAL_75 = ".७५"  # .75
...
class MeasureFst(GraphFst):
    ...
    Finite state transducer for classifying measure, suppletive aware, e.g.
        ...
        -१२.२kg -> measure { decimal { negative: "true"  integer_part: "बारह"  fractional_part: "दुइ"} units: "किलोग्राम" }
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst):
        ...
        # Convert Arabic digits to Magadhi for measures
        ...
        quarter = pynini.string_map(
            [
                (MAG_POINT_FIVE, MAG_SADHE),
                (MAG_ONE_POINT_FIVE, MAG_DEDH),
                (MAG_TWO_POINT_FIVE, MAG_DHAI),
            ]
        )
        ...
        dedh_dhai = pynini.string_map([(MAG_ONE_POINT_FIVE, MAG_DEDH), (MAG_TWO_POINT_FIVE, MAG_DHAI)])
        dedh_dhai_graph = pynutil.insert("integer: \"") + dedh_dhai + pynutil.insert("\"")

        savva_numbers = cardinal_graph + pynini.cross(MAG_DECIMAL_25, "")
        savva_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MAG_SAVVA)
            + pynutil.insert(NEMO_SPACE)
            + savva_numbers
            + pynutil.insert("\"")
        )

        sadhe_numbers = cardinal_graph + pynini.cross(MAG_POINT_FIVE, "")
        sadhe_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MAG_SADHE)
            + pynutil.insert(NEMO_SPACE)
            + sadhe_numbers
            + pynutil.insert("\"")
        )
        ...
        paune_numbers = paune + pynini.cross(MAG_DECIMAL_75, "")
        paune_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MAG_PAUNE)
            + pynutil.insert(NEMO_SPACE)
            + paune_numbers
            + pynutil.insert("\"")
        )
