...
with open(get_abs_path("data/date/suffixes.tsv"), "r", encoding="utf-8") as f:
    suffixes_list = [line.strip() for line in f if line.strip()]
with open(get_abs_path("data/date/prefixes.tsv"), "r", encoding="utf-8") as f:
    prefixes_list = [line.strip() for line in f if line.strip()]

# Create union of suffixes and prefixes
suffix_union = pynini.union(*suffixes_list) if suffixes_list else pynini.accep("")
prefix_union = pynini.union(*prefixes_list) if prefixes_list else pynini.accep("")


class DateFst(GraphFst):
    """
    Finite state transducer for classifying date, e.g.
        "०१-०४-२०२४" -> date { day: "एक" month: "अप्रैल" year: "दुइ हज़ार चौबीस" }
        "०४-०१-२०२४" -> date { month: "अप्रैल" day: "एक" year: "दुइ हज़ार चौबीस" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        from indic_text_normalization.text_normalization.mag.graph_utils import NEMO_DIGIT
        
        # Convert Arabic digits to Magadhi for dates
        ...
        # Support both Magadhi and Arabic digits for year patterns
        magadhi_year_thousands = pynini.compose(
            (NEMO_MAG_DIGIT + NEMO_MAG_ZERO + NEMO_MAG_DIGIT + NEMO_MAG_DIGIT), cardinal.graph_thousands
        )
        arabic_year_thousands = pynini.compose(
            (NEMO_DIGIT + pynini.accep("0") + NEMO_DIGIT + NEMO_DIGIT),
            arabic_to_magadhi_number @ cardinal.graph_thousands
        )
        graph_year_thousands = magadhi_year_thousands | arabic_year_thousands

        magadhi_year_hundreds_as_thousands = pynini.compose(
            (NEMO_MAG_DIGIT + NEMO_MAG_NON_ZERO + NEMO_MAG_DIGIT + NEMO_MAG_DIGIT), cardinal.graph_hundreds_as_thousand
        )
        arabic_year_hundreds_as_thousands = pynini.compose(
            (NEMO_DIGIT + pynini.union("1", "2", "3", "4", "5", "6", "7", "8", "9") + NEMO_DIGIT + NEMO_DIGIT),
            arabic_to_magadhi_number @ cardinal.graph_hundreds_as_thousand
        )
        graph_year_hundreds_as_thousands = magadhi_year_hundreds_as_thousands | arabic_year_hundreds_as_thousands

        # Support both Magadhi and Arabic digits for day/month
        magadhi_cardinal_graph = pynini.union(
        ...
        arabic_cardinal_graph = pynini.compose(
            ...
            arabic_to_magadhi_number @ magadhi_cardinal_graph
        )
        cardinal_graph = magadhi_cardinal_graph | arabic_cardinal_graph
        ...
        # Support both Magadhi and Arabic digits for days and months
        magadhi_days_graph = pynutil.insert("day: \"") + days + pynutil.insert("\"") + insert_space
        arabic_days_input = pynini.closure(NEMO_DIGIT, 1, 2)
        arabic_days_graph = pynini.compose(
            arabic_days_input,
            arabic_to_magadhi_number @ days
        )
        arabic_days_graph = pynutil.insert("day: \"") + arabic_days_graph + pynutil.insert("\"") + insert_space
        days_graph = magadhi_days_graph | arabic_days_graph

        magadhi_months_graph = pynutil.insert("month: \"") + months + pynutil.insert("\"") + insert_space
        arabic_months_input = pynini.closure(NEMO_DIGIT, 1, 2)
        arabic_months_graph = pynini.compose(
            arabic_months_input,
            arabic_to_magadhi_number @ months
        )
        arabic_months_graph = pynutil.insert("month: \"") + arabic_months_graph + pynutil.insert("\"") + insert_space
        months_graph = magadhi_months_graph | arabic_months_graph
        ...
        # Graph for year
        century_number = pynini.compose(pynini.closure(NEMO_MAG_DIGIT, 1), cardinal_graph) + pynini.accep("वाँ")
