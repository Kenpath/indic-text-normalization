...
from indic_text_normalization.text_normalization.gu.graph_utils import GraphFst, NEMO_DIGIT, insert_space
...
def get_quantity(decimal: 'pynini.FstLike', cardinal_up_to_hundred: 'pynini.FstLike') -> 'pynini.FstLike':
    ...
    e.g. ૧ લાખ -> integer_part: "એક" quantity: "લાખ"
    e.g. ૧.૫ લાખ -> integer_part: "એક" fractional_part: "પાંચ" quantity: "લાખ"
...
class DecimalFst(GraphFst):
    ...
    Finite state transducer for classifying decimal, e.g.
        -૧૨.૫૦૦૬ કરોડ -> decimal { negative: "true" integer_part: "બાર"  fractional_part: "પાંચ શૂન્ય શૂન્ય છ" quantity: "કરોડ" }
        ૧ કરોડ -> decimal { integer_part: "એક" quantity: "કરોડ" }
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Support both Gujarati and Arabic digits for fractional part
        gujarati_fractional = graph_digit + pynini.closure(insert_space + graph_digit)
        arabic_fractional = pynini.compose(
            pynini.closure(NEMO_DIGIT, 1) + pynini.closure(NEMO_DIGIT),
            arabic_to_gujarati_number @ (graph_digit + pynini.closure(insert_space + graph_digit))
        )
        self.graph = (gujarati_fractional | arabic_fractional).optimize()
