...
from indic_text_normalization.text_normalization.mai.utils import get_abs_path

MAI_ZERO_DIGIT = pynini.union("0", "०")
MAI_MOBILE_START_DIGITS = pynini.union("६", "७", "८", "९", "6", "7", "8", "9").optimize()
MAI_LANDLINE_START_DIGITS = pynini.union("२", "३", "४", "६", "2", "3", "4", "6").optimize()

delete_zero = pynutil.delete(MAI_ZERO_DIGIT)
...
def generate_mobile(context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    mobile_start_digit = pynini.union(MAI_MOBILE_START_DIGITS @ digits, MAI_MOBILE_START_DIGITS @ digit_to_word)
...
def get_landline(std_length: int, context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Filter cardinals to only include allowed digits
    landline_start_digit = pynini.union(MAI_LANDLINE_START_DIGITS @ digits, MAI_LANDLINE_START_DIGITS @ digit_to_word)
...
def get_context(keywords: pynini.Fst):
    ...
    all_digits = pynini.union(NEMO_MAI_DIGIT, NEMO_DIGIT)
