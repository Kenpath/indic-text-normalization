...
from indic_text_normalization.text_normalization.mr.utils import get_abs_path

# Time patterns specific to time tagger
MR_DOUBLE_ZERO = "००"
MR_TIME_FIFTEEN = ":१५"  # :15
MR_TIME_THIRTY = ":३०"  # :30
MR_TIME_FORTYFIVE = ":४५"  # :45
...
class TimeFst(GraphFst):
    ...
    Finite state transducer for classifying time, e.g.
        १२:३०:३०  -> time { hours: "बारा" minutes: "तीस" seconds: "तीस" }
        १:४०  -> time { hours: "एक" minutes: "चाळीस" }
        १:००  -> time { hours: "एक" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        # hour
        graph_h = self.hours + delete_colon + pynutil.delete(MR_DOUBLE_ZERO)

        dedh_adich_graph = pynini.string_map([("१" + MR_TIME_THIRTY, MR_DIDH), ("२" + MR_TIME_THIRTY, MR_ADICH)])

        savva_numbers = cardinal_graph + pynini.cross(MR_TIME_FIFTEEN, "")
        savva_graph = pynutil.insert(MR_SAVVA) + pynutil.insert(NEMO_SPACE) + savva_numbers

        sadhe_numbers = cardinal_graph + pynini.cross(MR_TIME_THIRTY, "")
        sadhe_graph = pynutil.insert(MR_SADHE) + pynutil.insert(NEMO_SPACE) + sadhe_numbers

        paune = pynini.string_file(get_abs_path("data/whitelist/paune_mappings.tsv"))
        paune_numbers = paune + pynini.cross(MR_TIME_FORTYFIVE, "")
        paune_graph = pynutil.insert(MR_PAUNE) + pynutil.insert(NEMO_SPACE) + paune_numbers
        ...
        final_graph = (
            ...
            | pynutil.add_weight(graph_dedh_adich, 0.1)
