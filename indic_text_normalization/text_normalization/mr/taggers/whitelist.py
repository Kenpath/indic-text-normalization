...
from indic_text_normalization.text_normalization.mr.utils import (
...
class WhiteListFst(GraphFst):
    def __init__(self, input_case: str, deterministic: bool = True, input_file: str = None):
        ...
        def _get_whitelist_graph(input_case, file, keep_punct_add_end: bool = False):
            ...
            # Filter out empty lines, lines with fewer than 2 columns, or entries with empty values
            whitelist = [label for label in whitelist if len(label) >= 2 and label[0].strip() and label[1].strip()]
