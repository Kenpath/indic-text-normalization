...
MR_ZERO_DIGIT = pynini.union("0", "०")
MR_MOBILE_START_DIGITS = pynini.union("६", "७", "८", "९").optimize()
MR_LANDLINE_START_DIGITS = pynini.union("२", "३", "४", "६").optimize()

delete_zero = pynutil.delete(MR_ZERO_DIGIT)
...
def generate_mobile(context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    mobile_start_digit = pynini.union(MR_MOBILE_START_DIGITS @ digits, MR_MOBILE_START_DIGITS @ digit_to_word)
...
def get_landline(std_length: int, context_keywords: pynini.Fst) -> pynini.Fst:
    ...
    # Filter cardinals to only include allowed digits
    landline_start_digit = pynini.union(MR_LANDLINE_START_DIGITS @ digits, MR_LANDLINE_START_DIGITS @ digit_to_word)
...
def get_context(keywords: pynini.Fst):
    ...
    all_digits = pynini.union(NEMO_MR_DIGIT, NEMO_DIGIT)
...
class TelephoneFst(GraphFst):
    ...
    Finite state transducer for tagging telephone numbers, e.g.
        ९१५७११४००७ -> telephone { number_part: "शून्य नऊ एक पाच सात एक एक चार शून्य शून्य सात" }
        +९१ ९२१०५१५६०६ -> telephone { country_code: "प्लस नऊ एक", number_part: "नऊ दोन एक शून्य पाच एक पाच सहा शून्य सहा" }
        १३७४-३०९९८८ -> telephone { number_part: "शून्य एक तीन सात चार तीन शून्य नऊ नऊ आठ आठ" }
    ...
    def __init__(self):
        ...
        graph = (
            pynutil.add_weight(mobile_number, 1.0)
