...
MR_POINT_FIVE = ".५"  # .5
MR_ONE_POINT_FIVE = "१.५"  # 1.5
MR_TWO_POINT_FIVE = "२.५"  # 2.5
MR_DECIMAL_25 = ".२५"  # .25
MR_DECIMAL_75 = ".७५"  # .75
...
class MeasureFst(GraphFst):
    ...
    Finite state transducer for classifying measure, suppletive aware, e.g.
        -१२kg -> measure { negative: "true" cardinal { integer: "बारा" } units: "किलोग्राम" }
        -१२.२kg -> measure { decimal { negative: "true"  integer_part: "बारा"  fractional_part: "दोन"} units: "किलोग्राम" }
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst):
        ...
        quarter = pynini.string_map(
            [
                (MR_POINT_FIVE, MR_SADHE),
                (MR_ONE_POINT_FIVE, MR_DIDH),
                (MR_TWO_POINT_FIVE, MR_ADICH),
            ]
        )
        ...
        dedh_adich = pynini.string_map([(MR_ONE_POINT_FIVE, MR_DIDH), (MR_TWO_POINT_FIVE, MR_ADICH)])
        dedh_adich_graph = pynutil.insert("integer: \"") + dedh_adich + pynutil.insert("\"")

        savva_numbers = cardinal_graph + pynini.cross(MR_DECIMAL_25, "")
        savva_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MR_SAVVA)
            + pynutil.insert(NEMO_SPACE)
            + savva_numbers
            + pynutil.insert("\"")
        )

        sadhe_numbers = cardinal_graph + pynini.cross(MR_POINT_FIVE, "")
        sadhe_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MR_SADHE)
            + pynutil.insert(NEMO_SPACE)
            + sadhe_numbers
            + pynutil.insert("\"")
        )
        ...
        paune_numbers = paune + pynini.cross(MR_DECIMAL_75, "")
        paune_graph = (
            pynutil.insert("integer: \"")
            + pynutil.insert(MR_PAUNE)
            + pynutil.insert(NEMO_SPACE)
            + paune_numbers
            + pynutil.insert("\"")
        )
