...
from indic_text_normalization.text_normalization.kn.graph_utils import MINUS, NEMO_NOT_QUOTE, GraphFst, delete_space


class CardinalFst(GraphFst):
    """
    Finite state transducer for verbalizing cardinal, e.g.
        cardinal { negative: "true" integer: "23" } -> ಋಣಾತ್ಮಕ ಇಪ್ಪತ್ತಮೂರು
    ...
    def __init__(self, deterministic: bool = True):
        ...
        if not deterministic:
            self.optional_sign |= pynini.cross("negative: \"true\"", MINUS)
            self.optional_sign |= pynini.cross("negative: \"true\"", " ಋಣ ")
