...
class PowerFst(GraphFst):
    ...
    Finite state transducer for classifying powers/exponents with superscripts, e.g.
        "10⁻⁷" -> power { base: "દસ" sign: "ઋણાત્મક" exponent: "સાત" }
        "2³" -> power { base: "બે" exponent: "ત્રણ" }
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Base number (regular digits - Gujarati or Arabic)
        gujarati_base_input = pynini.closure(NEMO_GU_DIGIT, 1)
        gujarati_base = pynini.compose(gujarati_base_input, cardinal_graph).optimize()
        
        arabic_base_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_to_gujarati = pynini.closure(
            pynini.string_map([
                ("0", "૦"), ("1", "૧"), ("2", "૨"), ("3", "૩"), ("4", "૪"),
                ("5", "૫"), ("6", "૬"), ("7", "૭"), ("8", "૮"), ("9", "૯")
            ])
        ).optimize()
        arabic_base = pynini.compose(arabic_base_input, arabic_to_gujarati @ cardinal_graph).optimize()
        
        base_number = gujarati_base | arabic_base
        ...
        optional_sign = pynini.closure(
            pynutil.insert('sign: "')
            + (
                pynini.cross(NEMO_SUPERSCRIPT_MINUS, "ઋણાત્મક")
                | pynini.cross(NEMO_SUPERSCRIPT_PLUS, "ધનાત્મક")
            )
            + pynutil.insert('"')
            + insert_space,
            0,
        ...
        exponent_value = pynini.compose(
            superscript_number,
            pynini.closure(superscript_to_digit) @ arabic_to_gujarati @ cardinal_graph
        ).optimize()
