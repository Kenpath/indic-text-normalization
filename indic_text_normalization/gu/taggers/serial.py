...
from indic_text_normalization.gu.utils import get_abs_path, load_labels
...
class SerialFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst, deterministic: bool = True, lm: bool = False):
        ...
        values with delimiters are handled by default).
            ...
            c325b -> tokens { cardinal { integer: "c ત્રણ બે પાંચ b" } }
        """
        # Gujarati characters
        GU_CHAR = pynini.union(
            *[chr(i) for i in range(0x0A80, 0x0AFF + 1)],  # Gujarati Unicode block
        ).optimize()
        
        alpha_num_seq = NEMO_ALPHA | GU_CHAR
        
        if deterministic:
            # For Gujarati, use final_graph for numbers
        ...
        if not deterministic:
            delimiter |= pynini.cross("-", " ડેશ ") | pynini.cross("/", " સ્લેશ ")
        ...
        # Note: In Gujarati, ordinals use Gujarati digits (૦-૯) while serials use ASCII digits (0-9)
        ...
        try:
            ...
            # This is acceptable since Gujarati ordinals and serials use different digit systems
            pass

        serial_graph = pynutil.add_weight(serial_graph, 0.0001)
        serial_graph |= (
            pynini.closure(NEMO_NOT_SPACE, 1)
            + (pynini.cross("^2", " વર્ગ") | pynini.cross("^3", " ઘન")).optimize()
        )
