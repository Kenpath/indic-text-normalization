...
class ScientificFst(GraphFst):
    ...
    Supported examples (Telugu):
      - "10.1-e5" -> scientific { mantissa: "పది దశాంశం ఒకటి" exponent: "ఐదు" }
      - "10.1e-5" -> scientific { mantissa: "పది దశాంశం ఒకటి" sign: "మైనస్" exponent: "ఐదు" }

    Verbalizer format (see `te/verbalizers/scientific.py`):
      mantissa + " గుణించి పది ఘాత " + [sign] + exponent
    """
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        telugu_int = pynini.compose(pynini.closure(NEMO_TE_DIGIT, 1), cardinal_graph).optimize()
        arabic_int = pynini.compose(pynini.closure(NEMO_DIGIT, 1), arabic_to_telugu_number @ cardinal_graph).optimize()
        integer_graph = (telugu_int | arabic_int).optimize()
        ...
        optional_sign = pynini.closure(
            pynutil.insert('sign: "')
            + (pynini.cross("-", "మైనస్") | pynini.cross("+", "ప్లస్"))
            + pynutil.insert('"')
            + insert_space,
            0,
            1,
