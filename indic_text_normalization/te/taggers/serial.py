...
from indic_text_normalization.te.utils import get_abs_path, load_labels
...
class SerialFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst, deterministic: bool = True, lm: bool = False):
        ...
        values with delimiters are handled by default).
            ...
            c325b -> tokens { cardinal { integer: "c మూడు రెండు ఐదు b" } }
        """
        if deterministic:
            # For Telugu, use final_graph for numbers
        ...
        serial_graph |= (
            ...
            + (pynini.cross("^2", " వర్గ") | pynini.cross("^3", " ఘన")).optimize()
        )
