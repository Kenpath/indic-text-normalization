...
class PowerFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Base number (regular digits - Maithili or Arabic)
        maithili_base_input = pynini.closure(NEMO_MAI_DIGIT, 1)
        maithili_base = pynini.compose(maithili_base_input, cardinal_graph).optimize()
        
        arabic_base_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_to_maithili = pynini.closure(
            pynini.string_map([
                ("0", "०"), ("1", "१"), ("2", "२"), ("3", "३"), ("4", "४"),
                ("5", "५"), ("6", "६"), ("7", "७"), ("8", "८"), ("9", "९")
            ])
        ).optimize()
        arabic_base = pynini.compose(arabic_base_input, arabic_to_maithili @ cardinal_graph).optimize()
        
        base_number = maithili_base | arabic_base
        ...
        exponent_value = pynini.compose(
            superscript_number,
            pynini.closure(superscript_to_digit) @ arabic_to_maithili @ cardinal_graph
        ).optimize()
