...
from indic_text_normalization.mai.graph_utils import GraphFst, NEMO_DIGIT, NEMO_MAI_DIGIT, insert_space
...
class ScientificFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        maithili_int = pynini.compose(pynini.closure(NEMO_MAI_DIGIT, 1), cardinal_graph).optimize()
        arabic_int = pynini.compose(pynini.closure(NEMO_DIGIT, 1), arabic_to_maithili_number @ cardinal_graph).optimize()
        integer_graph = (maithili_int | arabic_int).optimize()

        # Fractional digits spoken digit-by-digit
        maithili_frac = pynini.compose(
            pynini.closure(NEMO_MAI_DIGIT, 1),
            digit_word_graph + pynini.closure(insert_space + digit_word_graph),
        ).optimize()
        arabic_frac = pynini.compose(
            pynini.closure(NEMO_DIGIT, 1),
            arabic_to_maithili_number @ (digit_word_graph + pynini.closure(insert_space + digit_word_graph)),
        ).optimize()
        fractional_graph = (maithili_frac | arabic_frac).optimize()
