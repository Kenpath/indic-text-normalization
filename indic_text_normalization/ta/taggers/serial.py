...
from indic_text_normalization.ta.utils import get_abs_path, load_labels
...
class SerialFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst, deterministic: bool = True, lm: bool = False):
        ...
        values with delimiters are handled by default).
            ...
            c325b -> tokens { cardinal { integer: "c மூன்று இரண்டு ஐந்து b" } }
        """
        if deterministic:
            # For Tamil, use final_graph for numbers
        ...
        serial_graph |= (
            ...
            + (pynini.cross("^2", " வர்க்க") | pynini.cross("^3", " கன")).optimize()
        )
