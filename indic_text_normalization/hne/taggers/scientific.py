...
class ScientificFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Arabic digits -> Chhattisgarhi digits
        ...
        cg_int = pynini.compose(pynini.closure(NEMO_CG_DIGIT, 1), cardinal_graph).optimize()
        arabic_int = pynini.compose(pynini.closure(NEMO_DIGIT, 1), arabic_to_cg_number @ cardinal_graph).optimize()
        integer_graph = (cg_int | arabic_int).optimize()
        ...
        arabic_frac = pynini.compose(
            ...
            arabic_to_cg_number @ (digit_word_graph + pynini.closure(insert_space + digit_word_graph)),
        ).optimize()
