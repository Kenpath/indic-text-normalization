...
class PowerFst(GraphFst):
    ...
    Finite state transducer for classifying powers/exponents with superscripts, e.g.
        "10⁻⁷" -> power { base: "दस" sign: "माइनस" exponent: "सात" }
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Base number (regular digits - Bhojpuri or Arabic)
        bho_base_input = pynini.closure(NEMO_BHO_DIGIT, 1)
        bho_base = pynini.compose(bho_base_input, cardinal_graph).optimize()
        
        arabic_base_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_to_bho = pynini.closure(
            pynini.string_map([
                ("0", "०"), ("1", "१"), ("2", "२"), ("3", "३"), ("4", "४"),
                ("5", "५"), ("6", "६"), ("7", "७"), ("8", "८"), ("9", "९")
            ])
        ).optimize()
        arabic_base = pynini.compose(arabic_base_input, arabic_to_bho @ cardinal_graph).optimize()
        
        base_number = bho_base | arabic_base
        ...
        optional_sign = pynini.closure(
            pynutil.insert('sign: "')
            + (
                pynini.cross(NEMO_SUPERSCRIPT_MINUS, "माइनस")
                | pynini.cross(NEMO_SUPERSCRIPT_PLUS, "प्लस")
            )
            + pynutil.insert('"')
            + insert_space,
            0,
        ...
        exponent_value = pynini.compose(
            superscript_number,
            pynini.closure(superscript_to_digit) @ arabic_to_bho @ cardinal_graph
        ).optimize()
