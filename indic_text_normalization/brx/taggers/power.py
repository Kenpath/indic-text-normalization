...
class PowerFst(GraphFst):
    ...
    Finite state transducer for classifying powers/exponents with superscripts, e.g.
        "10⁻⁷" -> power { base: "स्से" sign: "ऋणात्मक" exponent: "सनि" }
        "2³" -> power { base: "नै" exponent: "थाम" }

    Args:
        cardinal: CardinalFst (uses Bodo number words: से, नै, थाम, स्से, etc.)
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        # Base number (regular digits - Bodo or Arabic)
        brx_base_input = pynini.closure(NEMO_BRX_DIGIT, 1)
        brx_base = pynini.compose(brx_base_input, cardinal_graph).optimize()
        
        arabic_base_input = pynini.closure(NEMO_DIGIT, 1)
        arabic_to_brx = pynini.closure(
            pynini.string_map([
                ("0", "०"), ("1", "१"), ("2", "२"), ("3", "३"), ("4", "४"),
                ("5", "५"), ("6", "६"), ("7", "७"), ("8", "८"), ("9", "९")
            ])
        ).optimize()
        arabic_base = pynini.compose(arabic_base_input, arabic_to_brx @ cardinal_graph).optimize()
        
        base_number = brx_base | arabic_base
        ...
        exponent_value = pynini.compose(
            superscript_number,
            pynini.closure(superscript_to_digit) @ arabic_to_brx @ cardinal_graph
        ).optimize()
