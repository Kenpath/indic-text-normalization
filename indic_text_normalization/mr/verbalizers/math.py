...
class MathFst(GraphFst):
    ...
    Finite state transducer for verbalizing math expressions, e.g.
        math { left: "एक" operator: "बरोबर" right: "दोन" } -> एक बरोबर दोन
    ...
    def __init__(self, deterministic: bool = True):
        ...
        left = (
            ...
            + delete_space
            + pynutil.delete("\"")
            + pynini.closure(NEMO_NOT_QUOTE, 0)  # Allow empty
            + pynutil.delete("\"")
        )
        ...
        # Using insert_space to ensure separation
        simple_expression = (
        ...
        # Operator with number (e.g., "+5" -> "प्लस पाच")
        operator_number_expression = operator + insert_space + right

        # Number with operator (e.g., "5*" -> "पाच गुणिले")
        number_operator_expression = left + insert_space + operator

        # Standalone operator (e.g., "+" -> "प्लस")
        # Matches when left/right are empty
